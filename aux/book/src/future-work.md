...
**Core language commitment:** string literals are `ptr`, always null-terminated (see Open
Questions §4). That guarantee is deliberately minimal — enough for `extern` FFI, nothing
richer.

**Decision:** a length-carrying string type lives in the standard library, not the
language:
...
**Deferred optimization — header-behind-pointer:** an alternative keeps a string as a
single `ptr` (no wrapper struct) by storing the length just *before* the pointed-to data,
so `p[0]` is still the first byte and length is `*(u32*)(p - 4)`. Not rejected, just
blocked: it needs pointer arithmetic and a cast operator, neither of which exist yet (see
Semantic Analysis → Uncertain checks). Worth revisiting once those land, if the extra
word per `Str` ever actually matters.
